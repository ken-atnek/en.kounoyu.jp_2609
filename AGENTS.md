...
4. TSXを触る場合は `docs/rules/tsx-comment-rules.md`
5. SCSSを触る場合は `docs/rules/scss-comment-rules.md`
6. 作業対象に関係する案件固有Markdown
7. SCSSを触る場合はSass / CSS共通プレイブック
...
- 構築段階の実装対象はPCのみとする
- スマートフォンのレスポンシブ調整はユーザーが行うため、CodexはSP用のレイアウト、余白、文字サイズ、表示切り替え、メニュー仕様、media queryを独自に追加・確定しない
